...
export default function SharePage() {
  ...
  useEffect(() => {
    ...
    if (params.get('step') === '3') {
      ...
      if (stored) { try { const loc = JSON.parse(stored); setSelected(new Set([loc.id])); sessionStorage.removeItem('sharePreselectedLocation') } catch {} }
      setStep(3)
    }
  }, [])
  ...
  useEffect(() => {
    supabase.from('locations').select('id,name,city,state,latitude,longitude,access_type,rating,save_count').eq('status','published').not('latitude','is',null).not('longitude','is',null).order('save_count',{ascending:false}).limit(500)
      .then(({ data }) => {
        if (!data) return
        setAllDbLocs(data.map((loc,idx) => ({
          ...
          type: 'favorite' as const, d: pin ? calcDist(pin.lat, pin.lng, loc.latitude, loc.longitude) : null,
        })))
      })
  ...
  const favorites: MapLocation[] = useMemo(() =>
    dbFavorites.filter(f => f.locations?.latitude && f.locations?.longitude).map((f,idx) => ({
      id: f.location_id, name: f.locations.name, city: f.locations.city,
      lat: f.locations.latitude, lng: f.locations.longitude,
      access: f.locations.access_type ?? 'public', rating: f.locations.rating?.toString() ?? '—',
      bg: `bg-${(idx%6)+1}`, type: 'favorite' as const,
  ...
  const secretLocs: MapLocation[] = useMemo(() =>
    dbSecrets.filter(s => s.lat && s.lng).map(s => ({
      ...
      lat: s.lat!, lng: s.lng!, access: 'public', rating: '—',
      bg: s.bg, type: 'secret' as const,
      d: pin ? calcDist(pin.lat, pin.lng, s.lat!, s.lng!) : null,
    })).sort((a,b) => (a.d??999)-(b.d??999)),
  [dbSecrets, pin])

  const allLocsWithDist: MapLocation[] = useMemo(() => allDbLocs.map(l => ({ ...l, d: pin ? calcDist(pin.lat,pin.lng,l.lat,l.lng) : null })), [allDbLocs, pin])
  ...
  const selectedLocs  = [...favorites, ...secretLocs, ...allDbLocs].filter((l,i,arr) =>
    arr.findIndex(x => String(x.id) === String(l.id)) === i && selected.has(l.id)
  )
  ...
  function toggleSelect(id: number | string) { setSelected(prev => { const next = new Set(prev); next.has(id) ? next.delete(id) : next.add(id); return next }) }
  function selectAllFavs() { setSelected(prev => { const next = new Set(prev); favsInRange.forEach(f => next.add(f.id)); return next }) }
  function goToStep(n: number) { if (n === 2 && !pin) return; if (n === 3 && selected.size === 0) return; if (n <= step || n === step + 1) setStep(n) }
  function nextStep() {
  ...
  async function generateLink() {
    ...
    try {
      ...
      const { error } = await supabase.from('share_links').insert({
        ...
        message: message.trim() || null, photographer_name: photographerName.trim() || null,
        my_photos_only: myPhotosOnly, location_ids: locationIds, secret_ids: secretIds, expires_at: expiresAt,
      })
      if (error) throw error
      setGeneratedSlug(slug); setStep(4); setToast('🔗 Share link created!')
    } catch (err) { console.error(err); setToast('⚠ Could not create link — please try again') }
  ...
  function resetAll() { setStep(1); setPin(null); setSelected(new Set()); setMyPhotosOnly(false); setGeneratedSlug(null); setSessionName(''); setMessage(''); setSelectedTemplate('') }
  ...
  function LocationRow({ loc, showTags = false }: { loc: MapLocation & { tags?: string[] }, showTags?: boolean }) {
    const sel = selected.has(loc.id), isSecret = loc.type === 'secret'
    return (
      <div onClick={() => toggleSelect(loc.id)} style={rowStyle(sel, isSecret)}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)', marginBottom: 2 }}>
            {loc.name}{isSecret && <span style={{ marginLeft: 6, fontSize: 10, color: '#7c5cbf', fontWeight: 500 }}>🤫 Secret</span>}
          </div>
          <div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>📍 {loc.city}</div>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          {loc.d !== null && loc.d !== undefined ? (<><div style={{ fontFamily: 'var(--font-playfair),serif', fontSize: 14, fontWeight: 600, color: 'var(--ink)' }}>{loc.d.toFixed(1)}</div><div style={{ fontSize: 10, color: 'var(--ink-soft)' }}>mi</div></>) : (<div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>★ {loc.rating}</div>)}
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="share-outer">
      ...
      {/* SIDEBAR */}
      <div className="share-sidebar" style={{ background: 'white', borderRight: '1px solid var(--cream-dark)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1.25rem 1.5rem 0', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button className="hamburger-btn" onClick={() => setMobileMenuOpen(p => !p)} style={{ background: 'rgba(26,22,18,.08)', border: '1px solid rgba(26,22,18,.15)', color: 'var(--ink)' }}>
                {mobileMenuOpen ? '✕' : '☰'}
              </button>
            </div>
          </div>
          ...
          {/* Mobile dropdown */}
          {mobileMenuOpen && (
            <div style={{ background: 'var(--cream)', border: '1px solid var(--cream-dark)', borderRadius: 8, padding: '0.75rem', marginBottom: '1rem' }}>
              <Link href="/dashboard" style={{ display: 'block', padding: '10px 0', fontSize: 14, color: 'var(--ink)', textDecoration: 'none', borderBottom: '1px solid var(--cream-dark)' }}>← Dashboard</Link>
              <Link href="/explore" style={{ display: 'block', padding: '10px 0', fontSize: 14, color: 'var(--ink)', textDecoration: 'none' }}>Browse map</Link>
            </div>
          )}

          <div style={{ fontFamily: 'var(--font-playfair),serif', fontSize: 22, fontWeight: 700, lineHeight: 1.15, color: 'var(--ink)', marginBottom: 4 }}>Share locations<br />with your client</div>
          <div style={{ fontSize: 13, color: 'var(--ink-soft)', fontWeight: 300, lineHeight: 1.55, marginBottom: '1.25rem' }}>Drop a pin, choose locations, and send a link.</div>
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid var(--cream-dark)', margin: '0 -1.5rem', padding: '0 1.5rem', overflowX: 'auto' }}>
            {STEP_LABELS.map((label, i) => {
              ...
              return (
                <div key={n} onClick={() => goToStep(n)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 0', marginRight: 12, fontSize: 12, fontWeight: 500, cursor: 'pointer', color: active ? 'var(--ink)' : done ? 'var(--sage)' : 'var(--ink-soft)', borderBottom: `2px solid ${active ? 'var(--gold)' : 'transparent'}`, whiteSpace: 'nowrap' }}>
                  <span style={{ width: 17, height: 17, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 600, background: active ? 'var(--gold)' : done ? 'var(--sage)' : 'var(--cream-dark)', color: active ? 'var(--ink)' : done ? 'white' : 'var(--ink-soft)' }}>{done ? '✓' : n}</span>
                  {label}
                </div>
              )
            })}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.25rem 1.5rem' }}>
          ...
          {step === 1 && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '1.25rem' }}>
                <div style={{ flex: 1, height: 1, background: 'var(--cream-dark)' }} /><span style={{ fontSize: 11, color: 'var(--ink-soft)', whiteSpace: 'nowrap' }}>then set radius</span><div style={{ flex: 1, height: 1, background: 'var(--cream-dark)' }} />
              </div>
              ...
              {dbSecrets.length > 0 && (
                <div style={{ marginTop: '1rem', padding: '10px 14px', background: 'rgba(124,92,191,.05)', border: '1px solid rgba(124,92,191,.2)', borderRadius: 8, display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--ink-soft)', lineHeight: 1.55, fontWeight: 300 }}>You have <strong style={{ fontWeight: 500, color: 'var(--ink)' }}>{dbSecrets.length} secret location{dbSecrets.length !== 1 ? 's' : ''}</strong> — they&apos;ll appear in Step 2.</div>
                </div>
              )}
            </>
          )}
          ...
          {step === 2 && (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <div><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)' }}>Your favorites</div><div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>{favsInRange.length} within {radius} mi</div></div>
              ...
              {favsInRange.length === 0 && favorites.length === 0 && <div style={{ fontSize: 13, color: 'var(--ink-soft)', fontStyle: 'italic', padding: '8px 0', marginBottom: 8 }}>No favorites — use &quot;Browse all&quot; below.</div>}
              ...
              {favsOutRange.length > 0 && (
                <>
                  ...
                  {favsOutRange.map(f => <div key={String(f.id)} style={{ ...rowStyle(false), opacity: .35, pointerEvents: 'none' }}><div style={checkStyle(false)} /><div className={f.bg} style={{ width: 42, height: 42, borderRadius: 6, flexShrink: 0 }} /><div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)' }}>{f.name}</div><div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>📍 {f.city}</div></div><div style={{ textAlign: 'right' }}><div style={{ fontFamily: 'var(--font-playfair),serif', fontSize: 14, fontWeight: 600 }}>{f.d?.toFixed(1) ?? '—'}</div><div style={{ fontSize: 10, color: 'var(--ink-soft)' }}>mi</div></div></div>)}
                </>
              )}
              ...
              <div style={{ marginTop: '1.5rem', borderTop: '1px solid var(--cream-dark)', paddingTop: '1.25rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: showAllLocs ? 10 : 0 }}>
                  <div><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)' }}>Browse all locations</div><div style={{ fontSize: 11, color: 'var(--ink-soft)', fontWeight: 300 }}>{allDbLocs.length} on the map</div></div>
                  <button onClick={() => setShowAllLocs(p => !p)} style={{ fontSize: 12, color: 'var(--gold)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: 0, fontWeight: 500 }}>{showAllLocs ? 'Hide ▲' : 'Browse ▼'}</button>
                </div>
                {showAllLocs && (
                  <>
                    <input type="text" value={locSearch} onChange={e => setLocSearch(e.target.value)} placeholder="Search by name or city…" style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--cream-dark)', borderRadius: 4, fontFamily: 'inherit', fontSize: 13, outline: 'none', marginBottom: 8, color: 'var(--ink)' }} />
                    <div style={{ maxHeight: 320, overflowY: 'auto' }}>
                      {allLocsWithDist.filter(l => { if (!locSearch.trim()) return true; const q = locSearch.toLowerCase(); return l.name.toLowerCase().includes(q) || l.city.toLowerCase().includes(q) }).slice(0,80).map(loc => <LocationRow key={String(loc.id)} loc={loc} />)}
                    </div>
                  </>
                )}
              </div>
          ...
          {step === 3 && (
            <>
              <div style={{ marginBottom: '1rem' }}><label style={labelStyle}>Session name *</label><input value={sessionName} onChange={e => setSessionName(e.target.value)} style={inputStyle} placeholder="e.g. Smith Family Fall Photos" /></div>
              <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ marginBottom: '1rem' }}><label style={labelStyle}>Your name / studio</label><input value={photographerName} onChange={e => setPhotographerName(e.target.value)} style={inputStyle} /></div>
              <div style={{ marginBottom: '1.25rem' }}>
              ...
              <div onClick={() => setMyPhotosOnly(p => !p)} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', background: myPhotosOnly ? 'rgba(196,146,42,.06)' : 'var(--cream)', border: `1px solid ${myPhotosOnly ? 'rgba(196,146,42,.3)' : 'var(--cream-dark)'}`, borderRadius: 8, marginBottom: '1.25rem', cursor: 'pointer' }}>
                ...
                <div><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)', marginBottom: 2 }}>Only show photos I uploaded</div><div style={{ fontSize: 12, color: 'var(--ink-soft)', fontWeight: 300, lineHeight: 1.5 }}>Client will only see your personal photos.</div></div>
              </div>
              <div style={{ padding: '12px 14px', background: 'var(--cream-dark)', borderRadius: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.07em', color: 'var(--ink-soft)', marginBottom: 8 }}>{selectedLocs.length} location{selectedLocs.length !== 1 ? 's' : ''} in this share</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                  {selectedLocs.map(l => <span key={String(l.id)} style={{ padding: '3px 9px', borderRadius: 20, fontSize: 11, background: l.type === 'secret' ? 'rgba(124,92,191,.1)' : 'white', color: l.type === 'secret' ? '#7c5cbf' : 'var(--ink-soft)', border: `1px solid ${l.type === 'secret' ? 'rgba(124,92,191,.2)' : 'var(--sand)'}` }}>{l.type === 'secret' && '🤫 '}{l.name}</span>)}
          ...
          {step === 4 && generatedSlug && (
            <>
              <div style={{ textAlign: 'center', padding: '0.5rem 0 1.25rem' }}>
                ...
                <div style={{ fontSize: 13, color: 'var(--ink-soft)', fontWeight: 300, lineHeight: 1.55 }}>Send this to your client.</div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'var(--cream)', border: '1px solid var(--sand)', borderRadius: 4, padding: '7px 7px 7px 12px', marginBottom: '1.25rem' }}>
                <span style={{ fontSize: 11, color: 'var(--sky)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'monospace' }}>{window.location.host}/pick/{generatedSlug}</span>
              ...
              <div style={{ background: 'var(--ink)', borderRadius: 10, padding: '1rem 1.1rem', display: 'flex', gap: 10, marginTop: '1rem' }}>
                ...
                <div style={{ fontSize: 12, color: 'rgba(245,240,232,.65)', lineHeight: 1.55, fontWeight: 300 }}><strong style={{ color: 'var(--cream)', fontWeight: 500 }}>You&apos;ll be notified by email</strong> the moment your client picks a location.</div>
              </div>
        ...
        <div style={{ padding: '.9rem 1.5rem', borderTop: '1px solid var(--cream-dark)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, background: 'white' }}>
          <button onClick={prevStep} style={{ visibility: step > 1 && step < 4 ? 'visible' : 'hidden', background: 'transparent', border: '1px solid var(--sand)', color: 'var(--ink-soft)', padding: '8px 16px', borderRadius: 4, fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>← Back</button>
          ...
          {step < 3 && <button onClick={nextStep} style={{ background: 'var(--gold)', color: 'var(--ink)', padding: '8px 16px', borderRadius: 4, border: 'none', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', opacity: (step === 1 && !pin) || (step === 2 && selected.size === 0) ? 0.4 : 1 }}>Next →</button>}
          {step === 3 && <button onClick={generateLink} disabled={!sessionName.trim() || isSaving} style={{ background: 'var(--gold)', color: 'var(--ink)', padding: '8px 16px', borderRadius: 4, border: 'none', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', opacity: !sessionName.trim() || isSaving ? 0.4 : 1 }}>{isSaving ? 'Saving…' : 'Generate →'}</button>}
        </div>
      </div>
      ...
      <div className="share-map-col">
        ...
        <div className="share-legend" style={{ background: 'white', borderRadius: 10, padding: '.9rem 1rem', border: '1px solid var(--cream-dark)', boxShadow: '0 4px 20px rgba(26,22,18,.1)', minWidth: 180 }}>
          ...
          {[{ color: '#c4922a', label: 'Client area pin' }, { color: '#4a6741', label: 'Selected' }, { color: '#7c5cbf', label: '🤫 Secret' }, { color: '#d4c9b0', label: 'Favorite' }, { color: '#3d352c', label: 'Out of range', dim: true }].map(item => (
