        ...
        {sent ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => setSent(true)} disabled={!message.trim()} style={{ flex: 1, padding: '9px', borderRadius: 4, background: 'var(--ink)', color: 'var(--cream)', border: 'none', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', opacity: message.trim() ? 1 : 0.4 }}>Send Report</button>
          ...
          {saved ? (
            ...
            <>
              <div style={{ marginBottom: '1.25rem' }}>
                ...
                {pin ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 13px', borderRadius: 6, marginTop: 8, background: 'rgba(74,103,65,.08)', border: '1px solid rgba(74,103,65,.2)', fontSize: 13, color: 'var(--sage)' }}>
                    <span>📍</span>
                    <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontWeight: 500 }}>Pin placed</div><div style={{ fontSize: 10, color: 'var(--sage)', marginTop: 1 }}>{pin.lat.toFixed(5)}, {pin.lng.toFixed(5)}</div></div>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {['public','private'].map(type => <button key={type} onClick={() => setAccessType(type)} style={{ flex: 1, padding: '9px', borderRadius: 4, border: `1.5px solid ${accessType === type ? 'var(--gold)' : 'var(--cream-dark)'}`, background: accessType === type ? 'rgba(196,146,42,.08)' : 'white', color: accessType === type ? 'var(--gold)' : 'var(--ink-soft)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>{type === 'public' ? '● Public' : '🔒 Private'}</button>)}
                </div>
              </div>
              <div style={{ marginBottom: '1.25rem' }}>
              ...
              <div style={{ borderTop: '1px solid var(--cream-dark)', paddingTop: '1.25rem', marginBottom: '1.25rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  <label style={{ ...labelStyle, marginBottom: 0 }}>Your photos <span style={{ fontSize: 10, fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: 'var(--ink-soft)' }}>(optional)</span></label>
                ...
                {photoFiles.length > 0 ? (
                  <>
                    ...
                    <input value={photoCaption} onChange={e => setPhotoCaption(e.target.value)} style={{ ...inputStyle, fontSize: 13, marginBottom: 8 }} placeholder="Caption (optional)" />
                    <div onClick={() => setPhotoPrivate(p => !p)} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 8, cursor: 'pointer', border: `1px solid ${photoPrivate ? 'rgba(124,92,191,.3)' : 'var(--cream-dark)'}`, background: photoPrivate ? 'rgba(124,92,191,.05)' : 'var(--cream)' }}>
                      ...
                      <div><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--ink)', marginBottom: 2 }}>🔒 Mark photos as private</div><div style={{ fontSize: 12, color: 'var(--ink-soft)', fontWeight: 300 }}>Only visible to you and your clients.</div></div>
                    </div>
                  </>
                ) : (
                  <div onClick={() => fileRef.current?.click()} style={{ border: '2px dashed var(--cream-dark)', borderRadius: 10, padding: '1.5rem', textAlign: 'center', cursor: 'pointer', background: 'var(--cream)' }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--ink-soft)', fontWeight: 300 }}>JPG, PNG · Max 10MB · Up to 10 photos</div>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={submit} disabled={saving || uploading || !name.trim() || !city.trim() || !state.trim() || !pin} style={{ flex: 1, padding: '12px', borderRadius: 4, background: 'var(--gold)', color: 'var(--ink)', border: 'none', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', opacity: saving || uploading || !name.trim() || !city.trim() || !state.trim() || !pin ? 0.5 : 1 }}>
                  {uploading ? `Uploading… (${uploaded}/${photoFiles.length})` : saving ? 'Submitting…' : !pin ? 'Search for location first' : photoFiles.length > 0 ? `Submit + ${photoFiles.length} photo${photoFiles.length !== 1 ? 's' : ''} →` : 'Submit location →'}
                </button>
    ...
    <>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 600, background: 'white', borderRadius: '16px 16px 0 0', zIndex: 500, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 -8px 48px rgba(26,22,18,.25)', animation: 'slideUp .28s ease' }}>
        ...
        <div style={{ position: 'relative', height: 220, background: '#1a1612', overflow: 'hidden' }}>
          ...
          {photoTab === 'upload' && <div className={loc.bg} style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 6 }}><span style={{ fontSize: 36 }}>📷</span><span style={{ fontSize: 13, color: 'rgba(255,255,255,.7)', fontWeight: 500 }}>Share your photos</span></div>}
        ...
        {photoTab === 'google' && <div style={{ padding: '6px 1.25rem', borderBottom: '1px solid var(--cream-dark)', display: 'flex', alignItems: 'center', gap: 6 }}><img src="https://developers.google.com/static/maps/documentation/images/google_on_white.png" alt="Google" style={{ height: 11, opacity: 0.4 }} /><span style={{ fontSize: 10, color: 'var(--ink-soft)' }}>Photos via Google Places · Not affiliated with LocateShoot</span></div>}
        ...
        <div style={{ padding: '1rem 1.25rem 1.5rem' }}>
          ...
          <div style={{ background: 'var(--cream)', border: '1px solid var(--cream-dark)', borderRadius: 8, padding: '12px 14px', marginBottom: '1rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>
              {loc.permit_website && <a href={loc.permit_website} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: 'var(--sky)', textDecoration: 'none', fontWeight: 500 }}>View official permit info →</a>}
              {loc.permit_scanned_at && <div style={{ fontSize: 10, color: 'var(--ink-soft)' }}>Last checked: {new Date(loc.permit_scanned_at).toLocaleDateString()}</div>}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, marginBottom: '1rem' }}>
            <button onClick={() => onToggleFavorite(loc.id as number)} style={{ flex: 1, padding: '12px', borderRadius: 4, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 500, background: isFav ? 'rgba(196,146,42,.1)' : 'var(--cream)', color: isFav ? 'var(--gold)' : 'var(--ink-soft)', border: `1px solid ${isFav ? 'rgba(196,146,42,.3)' : 'var(--cream-dark)'}`, transition: 'all .18s' }}>{isFav ? '♥ Saved' : '♡ Save to favorites'}</button>
          ...
          <div style={{ padding: '10px 12px', background: 'rgba(196,146,42,.04)', border: '1px solid rgba(196,146,42,.15)', borderRadius: 6, marginBottom: 10 }}>
            <div style={{ fontSize: 10, color: 'var(--ink-soft)', lineHeight: 1.6, fontWeight: 300 }}>⚠ <strong style={{ fontWeight: 500 }}>Disclaimer:</strong> LocateShoot provides location information for reference only. Always verify access rights, permit requirements, and safety conditions before your session. LocateShoot is not responsible for any access restrictions, fines, or incidents at listed locations.</div>
          </div>
...
export default function ExplorePage() {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if (e.key === 'Escape') { setDetailLoc(null); setShowAddModal(false); setShowFilters(false); setMobileMenuOpen(false) }
    }
  ...
  return (
    <div style={{ height: '100svh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.5rem', height: 56, background: 'rgba(26,22,18,.96)', backdropFilter: 'blur(8px)', borderBottom: '1px solid rgba(255,255,255,.07)', flexShrink: 0, zIndex: 200 }}>
        ...
        {/* Desktop search */}
        <div className="explore-nav-search" style={{ flex: 1, maxWidth: 400, margin: '0 1.5rem', position: 'relative' }}>
      ...
      {mobileMenuOpen && (
        <div className="mobile-menu" onClick={() => setMobileMenuOpen(false)}>
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search locations…" style={{ padding: '10px 12px', border: '1px solid rgba(255,255,255,.15)', borderRadius: 6, background: 'rgba(255,255,255,.08)', color: 'var(--cream)', fontFamily: 'inherit', fontSize: 14, outline: 'none', marginBottom: 8, width: '100%' }} onClick={e => e.stopPropagation()} />
          {user ? <Link href="/dashboard">Dashboard</Link> : <Link href="/">Join Free</Link>}
          <button onClick={() => { setShowAddModal(true); setMobileMenuOpen(false) }} style={{ fontSize: 15, color: 'rgba(245,240,232,.7)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: '12px 0', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,.06)' }}>+ Add a location</button>
          <button onClick={() => { requestLocation(); setMobileMenuOpen(false) }} style={{ fontSize: 15, color: 'rgba(245,240,232,.7)', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: '12px 0', textAlign: 'left' }}>📍 Near me</button>
        </div>
      )}
      ...
      <div style={{ background: 'white', borderBottom: '1px solid var(--cream-dark)', flexShrink: 0, zIndex: 100 }}>
        ...
        {showFilters && (
          <div style={{ padding: '1rem 1.5rem 1.25rem', borderTop: '1px solid var(--cream-dark)', background: 'var(--cream)', display: 'grid', gridTemplateColumns: 'auto 1fr auto auto', gap: '1.5rem', alignItems: 'start', overflowX: 'auto' }}>
            <div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                {ALL_TAGS.map(tag => <button key={tag} onClick={() => toggleTag(tag)} style={{ padding: '4px 10px', borderRadius: 20, fontSize: 11, fontWeight: 500, border: `1px solid ${selectedTags.includes(tag) ? 'var(--gold)' : 'var(--cream-dark)'}`, background: selectedTags.includes(tag) ? 'rgba(196,146,42,.12)' : 'white', color: selectedTags.includes(tag) ? 'var(--gold)' : 'var(--ink-soft)', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>{tag}</button>)}
              </div>
            </div>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {RATING_OPTIONS.map(opt => <button key={opt.value} onClick={() => setMinRating(opt.value)} style={{ padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500, border: `1px solid ${minRating === opt.value ? 'var(--gold)' : 'var(--cream-dark)'}`, background: minRating === opt.value ? 'rgba(196,146,42,.12)' : 'white', color: minRating === opt.value ? 'var(--gold)' : 'var(--ink-soft)', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>{opt.label}</button>)}
              </div>
            </div>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {SORT_OPTIONS.map(opt => <button key={opt.value} onClick={() => setSortBy(opt.value as SortValue)} style={{ padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 500, border: `1px solid ${sortBy === opt.value ? 'var(--gold)' : 'var(--cream-dark)'}`, background: sortBy === opt.value ? 'rgba(196,146,42,.12)' : 'white', color: sortBy === opt.value ? 'var(--gold)' : 'var(--ink-soft)', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>{opt.label}</button>)}
              </div>
            </div>
          </div>
        )}
      ...
      <div className="explore-body">
        ...
        <div className={`explore-sidebar${mobileMapVisible ? ' mobile-hidden' : ''}`}>
              ...
              <div key={loc.id} onClick={() => { setDetailLoc(loc); setActiveId(loc.id as number) }} style={{ display: 'flex', gap: 10, padding: '10px 1.25rem', borderBottom: '1px solid var(--cream-dark)', cursor: 'pointer', transition: 'background .15s', background: isActive ? 'rgba(196,146,42,.06)' : 'white', borderLeft: `3px solid ${isActive ? 'var(--gold)' : 'transparent'}` }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' }}>
                    ...
                    {(loc as any).permit_scanned_at && <span style={{ fontSize: 9, color: 'var(--ink-soft)', fontStyle: 'italic' }}>Permit checked {new Date((loc as any).permit_scanned_at).toLocaleDateString()}</span>}
                  </div>
                </div>
          ...
          {/* Extra bottom padding so last item isn't hidden by toggle button on mobile */}
          <div style={{ height: 80 }} />
        </div>

        {/* Map */}
        <div className={`explore-map-col${mobileMapVisible ? ' mobile-visible' : ''}`}>
          {/* Back to list button — only visible on mobile when map is shown */}
          {mobileMapVisible && (
            <button
              onClick={() => setMobileMapVisible(false)}
              style={{ position: 'absolute', top: 12, left: 12, zIndex: 500, display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 20, background: 'rgba(26,22,18,.9)', color: 'var(--cream)', border: '1px solid rgba(255,255,255,.15)', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', backdropFilter: 'blur(4px)' }}
            >
              ← List
            </button>
          )}
          ...
          <div style={{ position: 'absolute', bottom: 24, left: 16, zIndex: 500, background: 'white', borderRadius: 8, padding: '.75rem 1rem', border: '1px solid var(--cream-dark)', boxShadow: '0 4px 16px rgba(26,22,18,.1)' }}>
            {[{ color: '#4a6741', label: 'Public' }, { color: '#b54b2a', label: 'Private' }, { color: '#c4922a', label: 'Selected' }, { color: '#3d6e8c', label: 'You' }].map(item => (
              <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 11, color: 'var(--ink-mid)', marginBottom: 3 }}>
          ...
          {!dbLoading && locations.length > 0 && (
            <div style={{ position: 'absolute', top: 16, right: 16, zIndex: 500, background: 'rgba(26,22,18,.85)', backdropFilter: 'blur(4px)', borderRadius: 20, padding: '5px 12px', fontSize: 12, color: 'var(--cream)', border: '1px solid rgba(255,255,255,.1)' }}>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: '5rem', right: '1.5rem', background: 'var(--ink)', color: 'var(--cream)', padding: '10px 18px', borderRadius: 10, fontSize: 13, border: '1px solid rgba(255,255,255,.1)', zIndex: 9999, boxShadow: '0 8px 32px rgba(0,0,0,.3)', animation: 'toast-in .25s ease' }}>
          {toast}
        </div>
      )}
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          .explore-nav-search { display: none !important; }
        }
      `}</style>
    </div>
  )
