...
function ReportModal({ locName, locId, onClose }: { locName:string; locId:any; onClose:()=>void }) {
  ...
  return(<><div onClick={onClose} className="anim-backdrop-in" style={{position:'fixed',inset:0,background:'rgba(26,22,18,.5)',zIndex:600}}/><div className="anim-modal-in" style={{position:'fixed',top:'50%',left:'50%',transform:'translate(-50%,-50%)',background:'white',borderRadius:12,width:420,maxWidth:'92vw',padding:'1.5rem',zIndex:700,boxShadow:'0 20px 60px rgba(0,0,0,.25)'}}>
  ...
  return (
    <>
      ...
      <div className="anim-sheet-up" style={{position:'fixed',bottom:0,left:'50%',transform:'translateX(-50%)',willChange:'transform',contain:'layout style',width:'100%',maxWidth:600,background:'white',borderRadius:'16px 16px 0 0',zIndex:1800,maxHeight:'90svh',overflowY:'auto',boxShadow:'0 -8px 48px rgba(26,22,18,.25)'}}>
        ...
        >
          {/* Show whatever photos we have IMMEDIATELY. Order used to
              be: spinner while google loading → carousel → fallback,
              which meant portfolio pins whose user-uploaded photos
              had already landed still showed a spinner until the
              Google Places call finished (500ms–2s of dead air). Now
              carousel wins the moment googlePhotos (user + google
              merged) has anything, and the spinner only shows if
              google is still fetching AND we have no photos yet. */}
          {hasGoogle?(
